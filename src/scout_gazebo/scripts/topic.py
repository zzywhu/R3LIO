...
class LidarAngleTracker:
    def __init__(self):
        ...
        # 创建发布器用于输出基于TF的角度
        self.tf_angle_pub = rospy.Publisher('/velodyne_tf_angle', JointState, queue_size=10)
        
        # 订阅joint_states话题  订阅关节角度值
    ...
    def print_angle_info(self):
        if self.current_angle is not None and self.initial_angle is not None:
            ...
            try:
                ...
                # 标准化到[0, 2π)
                normalized_tf_angle = tf_angle % (2 * math.pi)
                if normalized_tf_angle < 0:
                    normalized_tf_angle += 2 * math.pi
                
                # 发布TF角度信息
                tf_angle_msg = JointState()
                tf_angle_msg.header.stamp = rospy.Time.now()
                tf_angle_msg.name = ['velodyne_tf_angle']
                tf_angle_msg.position = [normalized_tf_angle]
                self.tf_angle_pub.publish(tf_angle_msg)
                
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                pass
            
            # 仅每40次循环打印一次信息(每秒10次)，避免日志过多
            if hasattr(self, 'print_counter'):
                self.print_counter += 1
            else:
                self.print_counter = 0
                
            if self.print_counter % 40 == 0:
                # 显示角度信息
