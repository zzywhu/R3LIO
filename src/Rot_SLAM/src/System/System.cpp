...
void System::laserMapFovSegment() {
  ...
  V3D posLid = _stateIkfom.pos +
               _stateIkfom.rot.matrix() *
                   _stateIkfom.offset_T_L_I;  // twl=twi+Rwi*til=Twi*til
  ...
  float mov_dist = max(
      (_config._cubeLen - 2.0 * MOV_THRESHOLD * _config._detRange) * 0.5 * 0.9,
...
bool System::collectRasterAngles(const double &begTime, const double &endTime) {
  if ((_motorAngleBuf.front().first > begTime &&
       _measures.tMotorAngles.size() == 0) ||
...
bool System::getRasterAngle(const double &curTime) {
  ...
  if (curTime <= _motorAngleBuf.back().first) {
    ...
    if (deltaAngle < -M_PI)
      ...
      deltaAngle -= 2 * M_PI;
    _curMotorAngle =
        popAngle + (deltaAngle * (curTime - popTime) / (frontTime - popTime));
    if (_curMotorAngle < 0) _curMotorAngle += 2 * M_PI;
    if (_curMotorAngle > 2 * M_PI) _curMotorAngle -= 2 * M_PI;
    if (_initMotorAngle < 0) _initMotorAngle = _curMotorAngle;
    if (!_config._isMotorInitialized) {
      ...
      relTo.block<3, 3>(0, 0) =
          AngleAxisToRotationMatrix(Eigen::Vector3d(0, 0, rotAngle));  // To1o2
      std::cout << std::endl
                << "Motor rot angle:" << RAD2DEG(rotAngle) << std::endl;
      // relTo.block<3, 3>(0, 0) = AngleAxisToRotationMatrix(Eigen::Vector3d(0,
      // 0, _curMotorAngle - _initMotorAngle)); // To1o2
...
int System::matchKdTree(const PointCloudXYZI::Ptr &localCloudPtr,
                        const PointCloudXYZI::Ptr &globalCloudPtr,
  ...
  for (int i = 0; i < localCloudSize; i++) {
    ...
    kdTree.Nearest_Search(pointWorld, NUM_MATCH_POINTS, pointsNear,
                          pointSearchSqDis);
    ...
    if (estiPlane(pabcd, pointsNear, 0.1)) {
      float pd2 = pabcd(0) * pointWorld.x + pabcd(1) * pointWorld.y +
                  pabcd(2) * pointWorld.z + pabcd(3);
      ...
      if (s > 0.9) {
        ...
        matches.emplace_back(
            pl, pabcd, V3D(pointsNear[0].x, pointsNear[0].y, pointsNear[0].z),
            norm, 1 - 0.75 * fabs(pd2));
      }
    }
  }
...
void System::motorMotionCompensation() {
  ...
  for (auto it_kp = _measures.tMotorAngles.end() - 1;
       it_kp != _measures.tMotorAngles.begin(); it_kp--) {
    auto head = it_kp - 1;
    auto tail = it_kp;

    double deltaAngle = tail->second - head->second;
    double deltaTime = tail->first - head->first;
    if (deltaTime < 1e-9) continue;
    if (deltaAngle < -M_PI)
      deltaAngle += 2 * M_PI;
    else if (deltaAngle > M_PI)
      deltaAngle -= 2 * M_PI;
    ...
    for (; it_pcl->curvature / double(1000) > offsetTime; it_pcl--) {
      ...
      if (it_pcl == _measures.lidar->points.end() - 1) endAngle = curAngle;
      ...
      it_pcl->y = p(1);
      it_pcl->z = p(2);
      if (it_pcl == _measures.lidar->points.begin()) break;
    }
    if (it_pcl == _measures.lidar->points.begin()) break;
  }
}
...
void System::motorMotionCompensationZGforSIM() {
  ...
  // std::ofstream fFrameOfs =std::ofstream(string(ROOT_DIR) +
  // "PCD/"+std::to_string(_lidarBegTime)+".txt", std::ios::trunc |
  // std::ios::in); static std::ofstream fTotalOfs
  // =std::ofstream(string(ROOT_DIR) + "PCD/all.txt", std::ios::trunc |
  // std::ios::in);
  for (auto it_kp = _measures.tMotorAngles.end() - 1;
       it_kp != _measures.tMotorAngles.begin(); it_kp--) {
    auto head = it_kp - 1;
    ...
    double offsetTime = head->first - _measures.lidar_beg_time;
    // std::cout<<"motor:"<<offsetTime<<"-"<<tail->first -
    // _measures.lidar_beg_time<<std::endl;
    std::cout << "angle:" << head->second << "-" << tail->second << std::endl;
    for (; it_pcl->curvature / double(1000) > offsetTime; it_pcl--) {
      dt = it_pcl->curvature / double(1000) - offsetTime;
      curAngle = motorSpeed * dt + head->second;
      // std::cout<<"lid:"<<it_pcl->curvature / double(1000)
      // <<"->"<<curAngle<<std::endl;
      Eigen::Matrix3d R;
      Eigen::Vector3d t;
...
bool System::lidarMotionCompensationWithTimestamp(
    typename pcl::PointCloud<PointType>::Ptr &cloudInOut,
    const Eigen::Matrix4d &T12,
    float minScanDurationMs)  // T12 (from k+1 to k frame)
{
  typename pcl::PointCloud<PointType>::Ptr cloudTemp(
  ...
  // get the first and last time stamp (stored as curvature) of this scan (time
  // stamp unit: ms)
  for (int i = 0; i < totalSize; i++) {
    // LOG(INFO)<<cloudInOut->points[i].curvature;
    lastTimestamp =
        std::max(lastTimestamp, double(cloudInOut->points[i].curvature));
    firstTimestamp =
        std::min(firstTimestamp, double(cloudInOut->points[i].curvature));
  }
  scanDuration = lastTimestamp - firstTimestamp;
  if (scanDuration < minScanDurationMs) return false;

  Eigen::Quaterniond estimatedQuat12 =
      Eigen::Quaterniond(T12.block<3, 3>(0, 0));
  Eigen::Vector3d estimatedTranslation12 = T12.block<3, 1>(0, 3);

  for (int i = 0; i < totalSize; i++) {
    s = (cloudInOut->points[i].curvature - firstTimestamp) /
        scanDuration;  // curvature as time stamp //TODO: fix
    ...
    Eigen::Quaterniond dQuat = Eigen::Quaterniond::Identity().slerp(
        s, estimatedQuat12);  // Spherical linear interpolation (slerp) for
                              // quaternion
    Eigen::Vector3d dTranslation = s * estimatedTranslation12;
    Eigen::Vector3d tempPoint(cloudInOut->points[i].x, cloudInOut->points[i].y,
                              cloudInOut->points[i].z);
...
void System::predictAndUndistort(const MeasureGroup &meas,
                                 PointCloudXYZI::Ptr pclOut) {
  ...
  Eigen::Matrix4d predictTwl =
      curTwl * (velocity * _dt).inverse();  // Tw3=Tw2*T23(T21/dt21=T32/dt32)
  predictTwl /= predictTwl(3, 3);

  Eigen::Quaterniond estimatedQuat21 =
      Eigen::Quaterniond(deltaT21.block<3, 3>(0, 0));
  ...
  for (; itrPt != pclOut->points.begin(); itrPt--) {
    double s = (endOffsetTime - itrPt->curvature / double(1000)) /
               _dt;  // curvature as time stamp //TODO: fix
    // the first point (with earliest timestamp) would change the most
    // while the last point (with latest timestamp) would change the least
    Eigen::Quaterniond dQuat21 = Eigen::Quaterniond::Identity().slerp(
        s, estimatedQuat21);  // Spherical linear interpolation (slerp) for
                              // quaternion
...
void System::updateKdTreeMap(PointVector &pointsIn) {
  ...
  for (int i = 0; i < ptSize; i++) {
    if (!_nearestPoints[i].empty() && _isEKFInited) {
      ...
      mid_point.x = floor(pointsIn[i].x / _config._filterSizeMap) *
                        _config._filterSizeMap +
                    0.5 * _config._filterSizeMap;
      mid_point.y = floor(pointsIn[i].y / _config._filterSizeMap) *
                        _config._filterSizeMap +
                    0.5 * _config._filterSizeMap;
      mid_point.z = floor(pointsIn[i].z / _config._filterSizeMap) *
                        _config._filterSizeMap +
                    0.5 * _config._filterSizeMap;
      ...
      if (need_add) pointToAdd.push_back(pointsIn[i]);
    } else
  ...
  _addedPoints.insert(_addedPoints.end(), pointAdded1.begin(),
                      pointAdded1.end());
  _addedPoints.insert(_addedPoints.end(), pointAdded2.begin(),
                      pointAdded2.end());
...
void System::updateVoxelMapWithKdtree(PointVector &pointsIn) {
  ...
  for (int i = 0; i < ptSize; i++) {
    vector<float> pointSearchSqDis(NUM_MATCH_POINTS);
    PointVector &pointsNear = _nearestPoints[i];
    _ikdtree.Nearest_Search(pointsIn[i], NUM_MATCH_POINTS, pointsNear,
                            pointSearchSqDis);
    if (!pointsNear.empty()) {
      bool need_add = true;
      PointType downsample_result, mid_point;
...
void System::initVoxelMap() {
  ...
  _twl = _stateIkfom.rot * _stateIkfom.offset_T_L_I.matrix() +
         _stateIkfom.pos;  // twl=Twi*til
  ...
  for (size_t i = 0; i < _localSurfCloudPtr->size(); i++) {
    PointWithCov pv;
    pv.featType = Plane;
    pv.pl << _localSurfCloudPtr->points[i].x, _localSurfCloudPtr->points[i].y,
        _localSurfCloudPtr->points[i].z;
    pv.pi = _stateIkfom.offset_R_L_I * pv.pl + _stateIkfom.offset_T_L_I;
    if (pv.pi[2] == 0) pv.pi[2] = 0.001;
    // pv.pw << _Rwl * pv.pl + _twl;
    pv.pw = _stateIkfom.rot * pv.pi + _stateIkfom.pos;
    _curSurfPvList.push_back(pv);
  }
  if (_config._covType == 2) calcPointNeighCov(_curSurfPvList, 20);
  ...
  for (size_t i = 0; i < _localSurfCloudPtr->size(); i++) {
    if (_config._covType == 1) {
      ...
      _curSurfPvList[i].obsCov =
          _Rwl * _curSurfPvList[i].bodyCov * _Rwl.transpose() +
          _Rwl * pointCrossmat * _kf.get_P().block<3, 3>(3, 3) *
              pointCrossmat.transpose() * _Rwl.transpose() +
          _kf.get_P().block<3, 3>(0, 0);
    } else if (_config._covType == 2)
      _curSurfPvList[i].obsCov = _curSurfPvList[i].neighCov.block<3, 3>(0, 0);
  }
  TicToc time1;
  // std::cout << "Inited voxel map num: " << _curSurfPvList.size() <<
  // std::endl;
  buildVoxelMap(_curSurfPvList, _frameId, _config._voxelLength,
                _config._maxLayers, _config._layerPointSizeList,
                _config._maxPointsSize, _config._maxPointsSize,
                _config._minSurfEigenValue, _voxelSurfMap);
...
void System::updateVoxelMap() {
  ...
  for (size_t i = 0; i < _curSurfPvList.size(); i++) {
    _curSurfPvList[i].pi = _stateIkfom.offset_R_L_I * _curSurfPvList[i].pl +
                           _stateIkfom.offset_T_L_I;
    if (_curSurfPvList[i].pi[2] == 0) _curSurfPvList[i].pi[2] = 0.001;
    _curSurfPvList[i].pw =
        _stateIkfom.rot * _curSurfPvList[i].pi + _stateIkfom.pos;
    if (_config._covType == 1) {
      M3D pointCrossmat;
      pointCrossmat << SKEW_SYM_MATRX(_curSurfPvList[i].pl);
      calcBodyCov(_curSurfPvList[i], _config._rangingCov, _config._angleCov);
  ...
  //	TicToc time1;
  ::updateVoxelMap(_curSurfPvList, _frameId, _config._voxelLength,
                   _config._maxLayers, _config._layerPointSizeList,
                   _config._maxPointsSize, _config._maxPointsSize,
                   _config._minSurfEigenValue, _voxelSurfMap, _boxToDel);
  ...
  if (_rotDeltas[0] > 0.1 && _posDeltas[0] > 0.1) {
    ...
    for (int i = 1; i < size; i++) {
      if (_rotDeltas[i] > _rotDeltas[0] || _posDeltas[i] > _posDeltas[0])
        nIncreased++;
      if (_rotDeltas[i] > 1. || _posDeltas[i] > 1.) nLarge++;
    }
    // std::cout<<"nIncreased/nLarge:"<<nIncreased<<"/"<<nLarge<<std::endl;
    if ((float)nIncreased / size >= 0.4 || (float)nLarge / size >= 0.4)
      if (_rotDeltas.back() > 0.5 * _rotDeltas.front()) return true;
  }
  double matchRatio =
      (float)_matchSurfSizeList.back() / _localSurfCloudDownPtr->size();
...
void System::motorInitialize() {
  ...
  //        transCloudInMotorAxis(_measures.lidar, _measures.lidar, 0, Rol,
  //        tol); motorMotionCompensation();
  //        //transCloudInMotorAxis(_measures.lidar, _measures.lidar,
  //        _curMotorAngle, Eigen::Matrix3d::Identity(),
  //        Eigen::Vector3d::Zero());
  //
  ...
  //
  //        Eigen::Matrix3d
  //        Roo=AngleAxisToRotationMatrix(Eigen::Vector3d(0,0,rotAngle));
  //        Too.block<3,3>(0,0)=Roo;
  //
  //        Eigen::Matrix4d guessT=Tlo*Too*Tol;
  //        _Rwl=guessT.block<3,3>(0,0);
  //        _twl=guessT.block<3,1>(0,3);
  //        _stateCur.pos_end = -_Rwl* _stateCur.offset_R_L_I.transpose() *
  //        _stateCur.offset_T_L_I +_twl; // twi=-Rwl*Rli*til+twl=Twl*tli
  //        _stateCur.rot_end = _Rwl *
  //        _stateCur.offset_R_L_I.transpose();//Rwi=Rwl*Rli
  //    }

  *_localCloudPtr = *_measures.lidar;
  if (processCloudLSQ(_regPCL_VGICP) <= 0) return;

  if (_relToList.empty()) return;
  ////////calculate extrinsic/////////
  ...
  // TicToc calcTime;
  if (calcEXRotation(_relToList, _relTlList, _Rol, _tol, 10,
                     _config._maxInierError)) {
    _tol = Eigen::Vector3d();
    std::ofstream extrinsicOfs =
        std::ofstream(string(ROOT_DIR) + "result/MotorExtrinsic",
                      std::ios::trunc | std::ios::in);
    extrinsicOfs << std::fixed << std::setprecision(6) << _tol.x() << ","
                 << _tol.y() << "," << _tol.z() << std::endl;
    extrinsicOfs << std::fixed << std::setprecision(6)
                 << Quaterniond(_Rol).coeffs().transpose() << std::endl;
...
void System::loopClosing(state_ikfom &state) {
  if (_config._isLoopEn && _config._isImuInitialized) {
    ...
    PointCloudXYZI::Ptr localCloudPtr(new PointCloudXYZI()),
        localCloudDownPtr(new PointCloudXYZI());
    ...
    _loopCloser->inputFrameData(_frameId, _lidarEndTime, _Rwg, _twg,
                                localCloudPtr, localCloudDownPtr, curPvList,
                                _imuProcessor->_imuDatas, false, _globalGrav);
...
void System::loopClosing(StatesGroup &state) {
  if (_config._isLoopEn && _config._isImuInitialized) {
    ...
    // input frame info into loopclosing thread\
        //_loopCloser->getProcessLock().lock();
    _loopCloser->inputFrameData(_frameId, _lidarBegTime, _Rwg, _twg,
                                localCloudPtr, localCloudDownPtr, curPvList,
                                _imuProcessor->_imuDatas, false, _globalGrav);
    //_loopCloser->getProcessLock().unlock();
...
void System::updateFrontendMap() {
  ...
  for (auto &frameBlock : _loopCloser->getFrameBuffer()) {
    if (frameBlock->_uniqueId <= _loopCloser->lastCorrectBlockID()) continue;
    //  std::cout << "Frontend correct buffer block id:" <<
    //  frameBlock->_uniqueId << std::endl;
    frameBlock->_poseLo =
        _loopCloser->getLoopTrans() * frameBlock->_poseLo;  // Tw2'=Tw1'*T1'2'

    for (auto &pv : frameBlock->_pvList) {
      pv.pw = frameBlock->_poseLo.block<3, 3>(0, 0) * pv.pi +
              frameBlock->_poseLo.block<3, 1>(0, 3);
      totalSurfPvList.emplace_back(pv);
    }
  }
  // update voxelmap
  if (!totalSurfPvList.empty())
    ::updateVoxelMap(totalSurfPvList, _frameId, _config._voxelLength,
                     _config._maxLayers, _config._layerPointSizeList,
                     _config._maxPointsSize, _config._maxCovPointsSize,
                     _config._minSurfEigenValue, _loopCloser->getVoxelSurfMap(),
                     _boxToDel);

  if (!_voxelSurfMap.empty()) {
    TicToc time;
    std::cout << "voxel surf map size before loop closing: "
              << _voxelSurfMap.size() << std::endl;
    for (auto locOctItr : _voxelSurfMap) delete locOctItr.second;
    _voxelSurfMap.clear();
    _voxelSurfMap = _loopCloser->getVoxelSurfMap();
    // std::cout << "voxel surf map size after loop closing: " <<
    // _voxelSurfMap.size() << std::endl; printf("Delete old voxel map time: %f
    // ms\n", time.toc()); std::cout << "Voxel map updated by loop closing" <<
    // std::endl;
  }
  ...
  if (_ikdtree.Root_Node) {
    ...
    // update ikdtree
    // std::cout<<"updating kdtree map on
    // "<<LoopCloser::config()._frontendWinSize<<" frames"<<std::endl;
    const int &blockSize = _loopCloser->getFrameBlocks().size();
    int localBlockSize = LoopCloser::config()._frontendWinSize;
    if (localBlockSize < 0) localBlockSize = blockSize;
    ...
    for (auto &block :
         _loopCloser->getFrameBuffer()) {  // add buffer frame cloud in
                                           // loopclosing thread
      PointCloudXYZI::Ptr globalPcDown(new PointCloudXYZI());
      pcl::transformPointCloud(*block->_pcDown, *globalPcDown, block->_poseLo);
      _ikdtree.Add_Points(globalPcDown->points, true);
...
void System::updateLoopStatus(state_ikfom &state) {
  ...
  if (_loopCloser->isUpdated()) {
    ...
    _loopCloser->setUpdated(
        false);  // Allow Loop closer to detect frames in buffer
    ...
    Eigen::Matrix4d transTwi =
        transTwg *
        Tgi;  //_loopCloser->getFrameBlocks().back()->_poseLo*Tgi;//transTwg*Tgi;
...
void System::calibAndRefineLI() {
  _frameNum++;
  if (!_config._isImuInitialized && !_isDataAccumStart &&
      _stateCur.pos_end.norm() > 0.05) {
    printf(BOLDCYAN
           "[Initialization] Movement detected, data accumulation "
           "starts.\n\n\n\n\n" RESET);
  ...
  if (_config._isImuInitialized &&
      (_frameNum % _config._origOdomFreq * _config._cutFrameNum == 0) &&
      !_isOnlineCalibFinish) {
    double onlineCalibDuration = _lidarEndTime - _onlineCalibStartTime;
    onlineCalibDuration = onlineCalibDuration < _config._onlineRefineTime
                              ? onlineCalibDuration
                              : _config._onlineRefineTime;
    ...
    if (onlineCalibDuration > (_config._onlineRefineTime - 1e-6)) {
      ...
      fileoutCalibResult(_foutResult, _stateCur,
      ...
      cout << endl
           ...
           << BOLDGREEN << RESULT_FILE_DIR("Initialization_result.txt") << RESET
           << endl;
    }
  }
  ...
  if (!_config._isImuInitialized && !_isDataAccumFinished &&
    ...
    _initiatorLI->push_Lidar_CalibState(_stateCur.rot_end, _stateCur.bias_g,
                                        _stateCur.vel_end, _lidarEndTime);
    // Data Accumulation Sufficience Appraisal
    _isDataAccumFinished = _initiatorLI->data_sufficiency_assess(
        _jacoRot, _frameNum, _stateCur.bias_g, mutableConfig()._origOdomFreq,
        mutableConfig()._cutFrameNum);

    if (_isDataAccumFinished) {
      //            std::cout<<"Data accumulation Finished"<<std::endl;
      _initiatorLI->LI_Initialization(mutableConfig()._origOdomFreq,
                                      mutableConfig()._cutFrameNum,
                                      _timediffImuWrtLidar, _moveStartTime);
      ...
      _stateCur.pos_end = -_stateCur.rot_end *
                              _stateCur.offset_R_L_I.transpose() *
                              _stateCur.offset_T_L_I +
                          _stateCur.pos_end;  // twi=-Rwl*Rli*til+twl=Twl*tli
      _stateCur.rot_end =
          _stateCur.rot_end * _stateCur.offset_R_L_I.transpose();
      ...
      // _cutFrameNum = 1;
      mutableConfig()._timeLagIMUWtrLidar =
          _initiatorLI
              ->get_total_time_lag();  // Compensate IMU's time in the buffer
      for (int i = 0; i < _imuBuffer.size(); i++)
        _imuBuffer[i]->header =
            _imuBuffer[i]->header - _config._timeLagIMUWtrLidar;
      if (_config._udpateMethod == 0) {
        ...
        _imuProcessor->set_gyr_cov(
            V3D(_config._gyrCov, _config._gyrCov, _config._gyrCov));
        _imuProcessor->set_acc_cov(
            V3D(_config._accCov, _config._accCov, _config._accCov));
        _imuProcessor->set_gyr_bias_cov(
            V3D(_config._bGyrCov, _config._bGyrCov, _config._bGyrCov));
        _imuProcessor->set_acc_bias_cov(
            V3D(_config._bAccCov, _config._bAccCov, _config._bAccCov));
      }
...
bool System::assertDegeneracy() {
  ...
  if (ratio1 < 0.05 || ratio2 < 0.05) return true;
  return false;
}

void System::mapping() {
  if (_frameId < _config._nSkipFrames) return;

  if (_measures.lidar->empty()) return;

  // Motor data exists: transform local pcl into motor zero angle coordinate(let
  // pl be coord in motor zero axis)
  if (System::config()._isMotorInitialized && _curMotorAngle >= 0) {
    // TicToc time;
    if (_cloudAxisTransfer)  // for ZG device
      motorMotionCompensationZG();
    else  // other devices(extrinsic consist of lidar->motor(0 deg), motor(0
          // deg)->imu)
    {
      transCloudInMotorAxis(_measures.lidar, _measures.lidar, 0, _Rol, _tol);
      motorMotionCompensation();
      transCloudInMotorAxis(_measures.lidar, _measures.lidar, _curMotorAngle,
  ...
  //    pathOfs<<std::setprecision(6)<<std::fixed<<string(ROOT_DIR) +
  //    "PCD/"<<_lidarBegTime-_sensorTimeDiff<<".txt";
  //    ReadWriter::outputCloudFile(pathOfs.str(),*_measures.lidar);

  //    static std::ofstream fCloudOfs =std::ofstream(string(ROOT_DIR) +
  //    "PCD/all.txt", std::ios::trunc | std::ios::in); for(auto& pt:
  //    _measures.lidar->points)
  ...
  *_localCloudPtr = *_measures.lidar;
  ...
  //    // Mapping with imu
    ...
    if (_config._udpateMethod == 0) processCloudIKFoM();
    // processCloudESIKF();
    else
...
//            usleep(5000);
//        std::cout<<"last frameblock
//        uniqueID:"<<_loopCloser->getFrameBlocks().back()->_uniqueId<<std::endl;
//        _loopCloser->getProcessLock().lock();
//
...
//            double s = 1.0;
//            if(AbsouteOrientation::runCalSevenParams(targetFramePts,
//            sourceFramePts, _gpsConstraintList.size(), R, t, s))
//            {
//                _gnssTrans(0, 0) = R[0]; _gnssTrans(0, 1) = R[1];
//                _gnssTrans(0, 2) = R[2];_gnssTrans(0, 3) = t[0]; _gnssTrans(1,
//                0) = R[3]; _gnssTrans(1, 1) = R[4]; _gnssTrans(1, 2) =
//                R[5];_gnssTrans(1, 3) = t[1]; _gnssTrans(2, 0) = R[6];
//                _gnssTrans(2, 1) = R[7]; _gnssTrans(2, 2) = R[8];_gnssTrans(2,
//                3) = t[2]; std::cout<<"Gnss trans
//                matrix:\n"<<_gnssTrans<<std::endl; std::cout<<"Absoute
//                orientation scale:"<<s<<std::endl;
//            }
//
...
//                V3D
//                gnssPosTrans=_gnssTrans.block<3,3>(0,0)*gpsData->pos+_gnssTrans.block<3,1>(0,3);
//
//                PointType pt;
//                pt.x=gnssPosTrans.x();
//                pt.y=gnssPosTrans.y();
...
//                double
//                error=(gnssPosTrans-frameBlock->_poseLo.block<3,1>(0,3)).norm();
//                std::cout<<"Align error:"<<error<<std::endl;
//            }
//            pcl::PCDWriter pcdWriter;
//            pcdWriter.writeBinary(string(ROOT_DIR) + "PCD/gpsCloudTrans.pcd",
//            *gpsCloudTrans);
//
//        }
//        _loopCloser->getProcessLock().unlock();
...
void System::processGNSS() {
  ...
  if (_loopCloser && !_loopCloser->getFrameBlocks().empty()) {
    if (_lastGNSSData)
      if ((_measures.gnss->pos - _lastGNSSData->pos).norm() < 0.5) return;
    ...
    static std::vector<double> sourceFramePts, targetFramePts;
    if (!_isGnssInited) {
      for (size_t i = 0; i < 3; i++) targetFramePts.emplace_back(_twl(i));
      for (size_t i = 0; i < 3; i++)
        sourceFramePts.emplace_back(_measures.gnss->pos[i]);
      ...
      if (_gpsConstraintBuf.size() >= config()._gnssInitSize) {
        ...
        double t[3];
        double s = 1.0;
        if (AbsouteOrientation::runCalSevenParams(
                targetFramePts, sourceFramePts, _gpsConstraintBuf.size(), R, t,
                s)) {
          _gnssTrans(0, 0) = R[0];
          _gnssTrans(0, 1) = R[1];
          _gnssTrans(0, 2) = R[2];
    ...
    if (_isGnssInited) {
      ...
      while (!_gpsConstraintBuf.empty()) {
        ...
        SensorMsgs::GNSSData::Ptr gpsDataTrans(
            new SensorMsgs::GNSSData(*gpsData));
        gpsDataTrans->pos = _gnssTrans.block<3, 3>(0, 0) * gpsData->pos +
                            _gnssTrans.block<3, 1>(0, 3);

        _gpsConstraintBuf.pop_front();
        // std::cout<<"Add GNSS pos contraint to frame "<<frameID<<":
        // "<<gpsDataTrans->pos.transpose()<<std::endl;
        ...
        if (config()._isEnable3DViewer) _gnssQueue.emplace(gpsDataTrans);
      }
      _dispMutex.unlock();
      if (_loopCloser->config()._isOutputPCD) {
        pcl::PCDWriter pcdWriter;
        pcdWriter.writeBinary(string(ROOT_DIR) + "PCD/gnssCloud.pcd",
                              *gnssCloud);
      }
...
bool System::syncPackages(MeasureGroup &meas) {
  if (_lidarBuffer.empty()) return false;
  /*** push a lidar scan ***/
  if (!_isLidarPushed) {
    meas.lidar = _lidarBuffer.front();
    meas.lidar_beg_time = _timeBuffer.front();
    if (meas.lidar->points.size() <=
        _config._minFramePoint)  // frame points too little
    {
      ...
      if (LidarProcess::config()._lidarType == TIMOO16 &&
          (meas.lidar->points.back().curvature -
           ...
           0.5 * double(1000) /
               double(_config._origOdomFreq * _config._cutFrameNum))) {
      ...
      _scanNum++;
      _lidarEndTime = meas.lidar_beg_time +
  ...
  if (!_imuBuffer.empty() && _config._isMotorInitialized) {
    ...
    if (_lastImuTimestamp < _lidarEndTime) return false;
    ...
    while ((!_imuBuffer.empty()) && (imuTime < _lidarEndTime)) {
      ...
      if (imuTime > _lidarEndTime) break;
      ...
      if (_imuPreintegration) {
        if (imuTime <= _lidarEndTime) {
          if (_imuPreintegration->predictByImu(frontIMUData, _statePropagat)) {
            ...
            Pose6D imuPose = set_pose6d(
                imuTime - _lidarBegTime, frontIMUData->linear_acceleration,
                frontIMUData->angular_velocity, curState.velocity(),
                curState.position(), curState.R());
            _imuProcessor->pushIMUPose(imuPose);  // For cloud undistortion
            _stateCur = _statePropagat;
            // printf(REDPURPLE "[Preintergrate] " RESET);
            // std::cout << "Pos in World Frame   = " <<
            // _stateCur.pos_end.transpose() << std::endl;
            // PAUSE;
          }

          // std::cout<<std::setprecision(6)<<std::fixed<<imuTime<<":
          // "<<frontIMUData->orientation.coeffs().transpose()<<std::endl;
  ...
  if (!_motorAngleBuf.empty()) {
    if (!collectRasterAngles(
            meas.lidar_beg_time + meas.lidar->front().curvature / double(1000),
            meas.lidar_end_time)) {
      _lidarBuffer.pop_front();
      _timeBuffer.pop_front();
    ...
    if (!getRasterAngle(meas.lidar_end_time)) return false;
  ...
  if (!_gpsBuffer.empty()) {
    ...
    while ((!_gpsBuffer.empty())) {
      ...
      if (gpsTime > _lidarEndTime + 0.1) break;
  ...
  _timeBuffer.pop_front();
  _isLidarPushed = false;
  // std::cout << std::fixed << std::setprecision(6) << "Lidar end time: " <<
  // _lidarEndTime << endl;
  return true;
}
...
void System::collectMotorIMU() {
  while (!_motorAngleBufAll.empty()) {
    ...
    if (frontTime <= _lidarEndTime + 0.1) {
      ...
      _motorAngleBufAll.pop_front();
      // cout << "motorTimestamp: " << frontTime << ", _lidarBegTime: " <<
      // _lidarBegTime << endl;
    } else
      break;
  }
  ...
  while (!_imuBufferAll.empty()) {
    double frontTime = _imuBufferAll.front()->header + _sensorTimeDiff -
                       _timediffImuWrtLidar - _config._timeLagIMUWtrLidar;
    if (frontTime <= _lidarEndTime + 0.1) {
      ...
      if (!_config._isImuInitialized && !_isDataAccumFinished)
        _initiatorLI->push_ALL_IMU_CalibState(_imuBufferAll.front(),
                                              _config._meanAccNorm);
      _imuBufferAll.pop_front();
      // cout << "imuTimestamp: " << frontTime + _timediffImuWrtLidar +
      // _timeLagIMUWtrLidar << ", _lidarBegTime: " << _lidarBegTime << endl;
    } else
      break;
  }
...
void System::lidCallback(PPointCloud::Ptr cloudIn, double lidTime) {
  ...
  // cout << std::fixed << std::setprecision(6) << "Lidar beg time: " << lidTime
  // << endl;

  if (!_isTimediffSetFlg && abs(_lidarBegTime - _lastImuTimestamp) > 20. &&
      !_imuBuffer.empty()) {
  ...
  if (!_config._isCutFrame) {
    ...
    _lidarProcessor->processCutFrameCloud(cloudIn, lidTime, ptrQueue, timeQueue,
                                          _config._cutFrameNum, scanCount++);
...
void System::loadParams(const std::string &filePath) {
  ...
  _config._lidarCorrectFilePath =
      common["lidar_correction_file_path"].as<std::string>();
  ...
  _config._featureExtractSegNum =
      preprocess["feature_extract_seg_num"].as<int>();
  _config._nMergedFrames = preprocess["n_merge_frames"].as<int>();
  LidarProcess::mutableConfig()._pointFilterNum =
      preprocess["point_filter_num"].as<int>();
  LidarProcess::mutableConfig()._blindMin =
      preprocess["blind_min"].as<double>();
  LidarProcess::mutableConfig()._blindMax =
      preprocess["blind_max"].as<double>();
  ...
  LidarProcess::mutableConfig()._timeUnit =
      preprocess["timestamp_unit"].as<int>();
  ...
  LoopCloser::mutableConfig()._poseGraphOptimizationMethod =
      LoopClosing["PGO_method"].as<std::string>();
  LoopCloser::mutableConfig()._detectLoopMethod =
      LoopClosing["detect_method"].as<int>();
  LoopCloser::mutableConfig()._detectorConfigPath =
      LoopClosing["detector_config_path"].as<std::string>();
  LoopCloser::mutableConfig()._coolingSubmapNum =
      LoopClosing["cooling_submap_num"].as<int>();
  LoopCloser::mutableConfig()._numFrameLargeDrift =
      LoopClosing["num_frame_large_drift"].as<int>();
  LoopCloser::mutableConfig()._minSubmapIdDiff =
      LoopClosing["min_submap_id_diff"].as<int>();
  LoopCloser::mutableConfig()._maxSubMapAccuTran =
      LoopClosing["max_submap_tccu_tran"].as<double>();
  LoopCloser::mutableConfig()._maxSubMapAccuRot =
      LoopClosing["max_submap_accu_rot"].as<double>();
  LoopCloser::mutableConfig()._maxSubMapAccuFrames =
      LoopClosing["max_submap_accu_frames"].as<int>();
  LoopCloser::mutableConfig()._normalRadiusRatio =
      LoopClosing["normal_radius_ratio"].as<double>();
  LoopCloser::mutableConfig()._fpfhRadiusRatio =
      LoopClosing["fpfh_radius_ratio"].as<double>();
  LoopCloser::mutableConfig()._salientRadiusRatio =
      LoopClosing["salient_radius_ratio"].as<double>();
  LoopCloser::mutableConfig()._nonMaxRadiusRatio =
      LoopClosing["nonMax_radius_ratio"].as<double>();
  LoopCloser::mutableConfig()._voxelDownSize =
      LoopClosing["voxel_down_size"].as<double>();
  LoopCloser::mutableConfig()._vgicpVoxRes =
      LoopClosing["vgicp_vox_res"].as<double>();
  LoopCloser::mutableConfig()._isFramewisePGO =
      LoopClosing["is_frame_wise_pgo"].as<bool>();
  LoopCloser::mutableConfig()._isPgoIncremental =
      LoopClosing["is_pgo_incremental"].as<bool>();
  LoopCloser::mutableConfig()._isStoreFrameBlock =
      LoopClosing["is_store_frame"].as<bool>();
  LoopCloser::mutableConfig()._frameStoreInterval =
      LoopClosing["frame_store_interval"].as<int>();
  LoopCloser::mutableConfig()._isCorrectRealTime =
      LoopClosing["is_correct_realTime"].as<bool>();
  LoopCloser::mutableConfig()._isLoopDetectEn =
      LoopClosing["loop_detect_enable"].as<bool>();
  LoopCloser::mutableConfig()._isOutputPCD =
      LoopClosing["is_output_PCD"].as<bool>();
  LoopCloser::mutableConfig()._frontendWinSize =
      LoopClosing["frontend_win_size"].as<int>();
...
bool System::initSystem() {
  ...
  if (_config._udpateMethod == 0) {  // EKF method for state update
    ...
    if (_config._isImuInitialized) {  // IMU extrinsic exists
      _imuProcessor->LI_init_done =
          false;  // To init imu in ImuProcess::Process
      _imuProcessor->set_mean_acc_norm(_config._meanAccNorm);
      _imuProcessor->set_gyr_cov(
          V3D(_config._gyrCov, _config._gyrCov, _config._gyrCov));
      _imuProcessor->set_acc_cov(
      ...
      //            _cutFrameNum = 2;
      // std::cout << "Extrinsic is set, no need to initialization!" <<
      // std::endl; printState("Initialized States");
    ...
    if (!_config._isImuInitialized) {
      std::cerr << "Update method 0 doesn't support unknown extrinsic!"
                << std::endl;
      return false;
    }
    ...
    if (!_config._isImuInitialized) {
      std::cerr << "Update method doesn't support unknown extrinsic!"
                << std::endl;
      return false;
    }
  }

  if (config()._isCutFrame) mutableConfig()._cutFrameNum = 1;
  ...
  if (_config._matchMethod == 0)
    _kf.init_dyn_share(get_f, df_dx, df_dw,
                       std::bind(&System::hShareModelKdTree, this,
                                 std::placeholders::_1, std::placeholders::_2),
                       _config._nMaxInterations, epsi);
  else if (_config._matchMethod == 1)
    _kf.init_dyn_share(get_f, df_dx, df_dw,
                       std::bind(&System::hShareModelVoxelMap, this,
                                 std::placeholders::_1, std::placeholders::_2),
                       _config._nMaxInterations, epsi);
  ...
  _lidarProcessor->setDownsampleLeafSize(_config._filterSizeSurf,
                                         _config._filterSizeSurf,
                                         _config._filterSizeSurf);
  _ikdtree.set_downsample_param(_config._filterSizeMap);

  if (_config._udpateMethod >= 1)
    _voxelMapGaussian.reset(new GaussianVoxelMap<PointType>(
        System::mutableConfig()._voxelLength, VoxelAccumulationMode::ADDITIVE));

  if (!_config._pcapFilePath.empty()) {  // Reading lidar data from pcap file
    double startAngle = 0.;
    //_pandarReader = new PandarGeneral(_config._pcapFilePath,
    // std::bind(&System::lidCallback, this, std::placeholders::_1,
    // std::placeholders::_2), static_cast<int>(startAngle * 100 + 0.5), 0, 0,
    //"PandarXT-16", "PandarXT-16", "", false); if (!_pandarReader)
    //{
    //    std::cout << "Created pander reader failed" << std::endl;
    //    return false;
    // }
    // if (0 !=
    // _pandarReader->LoadCorrectionFile(_config._lidarCorrectFilePath))
    // {
    //    std::cout << "Loaded correction file from " <<
    //    _config._lidarCorrectFilePath << " failed" << std::endl; return false;
    //}
  }
  ...
  {
    // Keep BAdata/ but clear per-run outputs
    const std::string baDir = std::string(ROOT_DIR) + "BAdata";
    const std::string baAllPcdBodyDir = baDir + "/all_pcd_body";
    std::error_code ec;
    std::filesystem::create_directories(baAllPcdBodyDir, ec);
    ReadWriter::cleanDir(baAllPcdBodyDir);

    // Truncate pose log at startup (will be appended during runtime)
    std::ofstream pose_ofs(baDir + "/lidar_poses.txt",
                           std::ios::out | std::ios::trunc);
  }

  std::cout << "Initialize|Lidar type: " << LidarProcess::config()._lidarType
            << endl;
...
bool System::start() {
  ...
  if (!ReadWriter::parseMotorFile(System::config()._rasterFilePath,
                                  _motorAngleBufAll)) {
...
void System::stop() {
  ...
  if (!_loopCloser->isFinished()) {
    ...
    while (!_loopCloser->isFinished()) usleep(5000);
  }
  _isFinished = true;
}
...
pcl::PointCloud<PointType>::Ptr lastSubmapCloud(
    new pcl::PointCloud<PointType>()),
    curTransCloud(new PointCloudXYZI);
PointCloudXYZI::Ptr curTransCloudDS(new PointCloudXYZI),
    lastSubmapCloudDS(new pcl::PointCloud<PointType>());
bool System::processContinualTask(const std::string &lastTaskPath) {
  if (!_loopCloser) return false;
  if (_loopCloser->getSubMapBlocks().empty()) return false;
  ...
  pcl::transformPointCloud(*_loopCloser->getSubMapBlocks().front()->_pcDown,
                           *curSubmapCloud,
                           _loopCloser->getSubMapBlocks().front()->_poseLo);
  float x, y, z, roll, pitch, yaw;
  pcl::getTranslationAndEulerAngles(
      Eigen::Affine3d(_loopCloser->getSubMapBlocks().front()->_poseLo)
          .cast<float>(),
      x, y, z, roll, pitch, yaw);

  Eigen::Affine3f transGuess =
      pcl::getTransformation(lastTaskRefPose[0] - x, lastTaskRefPose[1] - y,
                             lastTaskRefPose[2] - z, 0, 0, 0);
  ...
  pcdWriter.writeBinary(string(ROOT_DIR) + "/PCD/lastSubmapCloud.pcd",
                        *lastSubmapCloud);
  ...
  cReg.coarse_reg_SACIA(curTransCloudDS, lastSubmapCloudDS, coarseTransMatrix,
                        0 /*0.5*/, 30 /*30*/, 80);
  printf("续扫coarse拼接完成....\n");
  //        printf("续扫coarse拼接完成....\n");

  pcl::PointCloud<PointType>::Ptr coarseTransCloud(
      new pcl::PointCloud<PointType>());
  pcl::transformPointCloud(*curTransCloud, *coarseTransCloud,
                           coarseTransMatrix);
  pcdWriter.writeBinary(string(ROOT_DIR) + "/PCD/curTrans1.pcd",
                        *coarseTransCloud);

  std::cout << "fine align...\n";

  //    Eigen::Matrix4f  fineTransMatrix =  Eigen::Matrix4f::Identity();
  //    pcl::PointCloud<PointType>::Ptr finalTransCloud(new
  //    pcl::PointCloud<PointType>()); registerByGICP(coarseTransCloud,
  //    lastSubmapCloud, fineTransMatrix, 1.5, 10, finalTransCloud);
  //    pcdWriter.writeBinary(string(ROOT_DIR) + "/PCD/curTrans2.pcd",
  //    *finalTransCloud); Eigen::Matrix4d transCur2LastTask =
  //    Eigen::Matrix4f(fineTransMatrix*coarseTransMatrix *
  //    transGuess.matrix()).cast<double>();
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr localCloudXYZPtr2(
      new pcl::PointCloud<pcl::PointXYZ>);
  pcl::PointCloud<pcl::PointXYZ>::Ptr localCloudXYZPtr1(
      new pcl::PointCloud<pcl::PointXYZ>);
  ...
  Eigen::Matrix4d transCur2LastTask =
      fineTransMatrix *
      Eigen::Matrix4f(coarseTransMatrix * transGuess.matrix()).cast<double>();

  PointCloudXYZI::Ptr finalTransCloud(new PointCloudXYZI);
  pcl::transformPointCloud(*curSubmapCloud, *finalTransCloud,
                           transCur2LastTask);
  pcdWriter.writeBinary(string(ROOT_DIR) + "/PCD/finalTransCloud.pcd",
                        *finalTransCloud);

  //    for(auto & frameBlock : _loopCloser->getFrameBlocks())
  //    {
  //        pcl::PointCloud<PointType>::Ptr frameCloud(new
  //        pcl::PointCloud<PointType>());
  //        pcl::io::loadPCDFile(frameBlock->_pcdFilePath, *frameBlock->_pcRaw);
  //        pcl::transformPointCloud(*frameBlock->_pcRaw, *frameCloud,
  //        frameBlock->_poseLo); pcdWriter.writeBinary(string(ROOT_DIR) +
  //        "/PCD/frame_ori_"+std::to_string(frameBlock->_uniqueId)+".pcd",
  //        *frameCloud);
  //    }
...
void System::saveLastTaskInfo(const std::string &lastTaskPath) {
  if (_loopCloser->getSubMapBlocks().empty()) return;
  if (_loopCloser->getSubMapBlocks().size() < 2) return;

  auto lastSubmap = _loopCloser->getSubMapBlocks().back();

  float x, y, z, roll, pitch, yaw;
  pcl::getTranslationAndEulerAngles(
      Eigen::Affine3d(lastSubmap->_poseLo).cast<float>(), x, y, z, roll, pitch,
      yaw);
  ...
  if (fout) {
    ...
    double gravAlign[4] = {rotAlignQuat.x(), rotAlignQuat.y(), rotAlignQuat.z(),
                           rotAlignQuat.w()};
    ...
    PointCloudXYZI::Ptr globalDownCloud(new PointCloudXYZI());
    pcl::transformPointCloud(*lastSubmap->_pcDown, *globalDownCloud,
                             lastSubmap->_poseLo);
  ...
  //        pcl::transformPointCloud(*frameBlock->_pcRaw, *frameCloud,
  //        frameBlock->_poseLo); pcdWriter.writeBinary(string(ROOT_DIR) +
  //        "/PCD/frame_"+std::to_string(frameBlock->_uniqueId)+".pcd",
  //        *frameCloud);
  //    }
  std::cout << "Last task saved!" << std::endl;
}

bool System::readLastTaskInfo(const std::string &lastTaskPath, float *refXYZ,
