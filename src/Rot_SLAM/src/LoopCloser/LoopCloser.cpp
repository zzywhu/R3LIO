...
bool LoopCloser::startThread() {
  ...
  if (!_loopClosingThread) return false;
  return true;
}
...
void LoopCloser::release() {
  ...
  if (!_frameBuffer.empty()) _frameBuffer.clear();
...
void LoopCloser::loadDetectorConfig(const std::string& configPath) {
  ...
  yl.loadOneConfig({"ContourDBConfig", "max_fine_opt_"},
                   _dbConfig.max_fine_opt_);
  yl.loadSeqConfig({"ContourDBConfig", "q_levels_"}, _dbConfig.q_levels_);

  yl.loadOneConfig({"ContourDBConfig", "TreeBucketConfig", "max_elapse_"},
                   _dbConfig.tb_cfg_.max_elapse_);
  yl.loadOneConfig({"ContourDBConfig", "TreeBucketConfig", "min_elapse_"},
                   _dbConfig.tb_cfg_.min_elapse_);

  yl.loadOneConfig({"ContourDBConfig", "ContourSimThresConfig", "ta_cell_cnt"},
                   _dbConfig.cont_sim_cfg_.ta_cell_cnt);
  yl.loadOneConfig({"ContourDBConfig", "ContourSimThresConfig", "tp_cell_cnt"},
                   _dbConfig.cont_sim_cfg_.tp_cell_cnt);
  yl.loadOneConfig({"ContourDBConfig", "ContourSimThresConfig", "tp_eigval"},
                   _dbConfig.cont_sim_cfg_.tp_eigval);
  yl.loadOneConfig({"ContourDBConfig", "ContourSimThresConfig", "ta_h_bar"},
                   _dbConfig.cont_sim_cfg_.ta_h_bar);
  yl.loadOneConfig({"ContourDBConfig", "ContourSimThresConfig", "ta_rcom"},
                   _dbConfig.cont_sim_cfg_.ta_rcom);
  yl.loadOneConfig({"ContourDBConfig", "ContourSimThresConfig", "tp_rcom"},
                   _dbConfig.cont_sim_cfg_.tp_rcom);
  _ptrContourDB = std::make_unique<ContourDB>(_dbConfig);

  yl.loadOneConfig({"thres_lb_", "i_ovlp_sum"},
                   _thresLb.sim_constell.i_ovlp_sum);
  yl.loadOneConfig({"thres_lb_", "i_ovlp_max_one"},
                   _thresLb.sim_constell.i_ovlp_max_one);
  yl.loadOneConfig({"thres_lb_", "i_in_ang_rng"},
                   _thresLb.sim_constell.i_in_ang_rng);
  ...
  yl.loadOneConfig({"thres_lb_", "neg_est_dist"},
                   _thresLb.sim_post.neg_est_dist);

  yl.loadOneConfig({"thres_ub_", "i_ovlp_sum"},
                   _thresUb.sim_constell.i_ovlp_sum);
  yl.loadOneConfig({"thres_ub_", "i_ovlp_max_one"},
                   _thresUb.sim_constell.i_ovlp_max_one);
  yl.loadOneConfig({"thres_ub_", "i_in_ang_rng"},
                   _thresUb.sim_constell.i_in_ang_rng);
  ...
  yl.loadOneConfig({"thres_ub_", "neg_est_dist"},
                   _thresUb.sim_post.neg_est_dist);
  ...
  yl.loadOneConfig({"ContourManagerConfig", "lidar_height_"},
                   _cmConfig.lidar_height_);
  yl.loadOneConfig({"ContourManagerConfig", "blind_sq_"}, _cmConfig.blind_sq_);
  yl.loadOneConfig({"ContourManagerConfig", "min_cont_key_cnt_"},
                   _cmConfig.min_cont_key_cnt_);
  yl.loadOneConfig({"ContourManagerConfig", "min_cont_cell_cnt_"},
                   _cmConfig.min_cont_cell_cnt_);

  yl.close();
}

void LoopCloser::inputGNSSData(SensorMsgs::GNSSData::Ptr gnssData,
                               const int& frameID) {
...
void LoopCloser::inputFrameData(int frameId, const double& timeStamp,
                                ...
                                bool isNormalizeIntensity, V3D gravity) {
  _mutexBuf.lock();
  ...
  frameBlockPtr->_pcRaw =
      localCloud->makeShared();  // frame cloud in IMU coordinate
  frameBlockPtr->_pcDown =
      localCloudDown
          ->makeShared();  // frame downsampled cloud in IMU coordinate
  ...
  if (isNormalizeIntensity) {
    ...
    for (int i = 0; i < frameBlockPtr->_pcDown->points.size(); i++) {
      min_intensity =
          min_(min_intensity, frameBlockPtr->_pcDown->points[i].intensity);
      max_intensity =
          max_(max_intensity, frameBlockPtr->_pcDown->points[i].intensity);
    }
    float intesnity_scale =
        255.0 / (max_intensity - min_intensity);  // rescale to 0-255
    for (int i = 0; i < frameBlockPtr->_pcDown->points.size(); i++)
      frameBlockPtr->_pcDown->points[i].intensity =
          (frameBlockPtr->_pcDown->points[i].intensity - min_intensity) *
          intesnity_scale;
  }
...
void LoopCloser::storeFrameData() {
  ...
  if (LoopCloser::mutableConfig()._issavemap) {
    ...
    _curFrameBlock->_pcdFilePath = std::string(ROOT_DIR) + "PCD/" +
                                   std::to_string(_curFrameBlock->_uniqueId) +
                                   ".pcd";
    _pcdWriter.writeBinary(_curFrameBlock->_pcdFilePath,
                           *_curFrameBlock->_pcRaw);

    const std::string baDir = std::string(ROOT_DIR) + "BAdata/all_pcd_body/";
    std::error_code ec;
    std::filesystem::create_directories(baDir, ec);

    std::ostringstream ts_ss;
    ts_ss.setf(std::ios::fixed);
    ts_ss << std::setprecision(6) << _curFrameBlock->_timestamp;
    const std::string baPcdPath = baDir + ts_ss.str() + ".pcd";
    _pcdWriter.writeBinary(baPcdPath, *_curFrameBlock->_pcRaw);
  }
  // add to gtsam graph
  if (!_frameBlocks.empty()) {
    Eigen::Matrix4d relPose = _frameBlocks.back()->_poseLo.inverse() *
                              _curFrameBlock->_poseLo;  // T12=T1w*Tw2
    _relPoses.push_back(relPose);
  }

  _frameBlocks.emplace_back(new CloudBlock(*_curFrameBlock, true));
  _frameIDMap[_curFrameBlock->_uniqueId] =
      _curFrameBlock
          ->_idInStrip;  // for indexing frame in frameblocks by frame id

  if (config()._isPgoIncremental) {  // PGO incremental for a existed global
                                     // pose graph
    if (config()._isFramewisePGO) {
      if (_frameBlocks.size() == 1)
        _pgOptimizer.addFixedConstraint(_curFrameBlock);
      else {  // Add adjacent frame Edges to gtsam graph
        constraints adjCons;
        _confinder.add_adjacent_constraint(_frameBlocks, adjCons,
                                           _frameBlocks.size());
  ...
  if (!config()._isFramewisePGO &&
      !config()._isPgoIncremental) {  // no need to store cloud for
                                      // reconstructing map after submap PGO
    std::vector<PointWithCov>().swap(_frameBlocks.back()->_pvList);
    _frameBlocks.back()->_pcDown.reset(new PointCloudXYZI());
  } else {  // no need to store cloud outside frontend window for reconstructing
            // map after frame PGO
...
bool LoopCloser::detectLoop() {
  ...
  if (_isNewSubMap)  // first cloud block input
  {
    ...
    if (_config._isFramewisePGO || _config._isStoreFrameBlock)
      storeFrameData();  // Force to store when is the first frame of a new
                         // submap
    ...
    if (config()._isPgoIncremental) {  // PGO incremental for a existed global
      ...
      if (!config()._isFramewisePGO) {
        ...
        if (_keyframeBlocks.size() == 1)
          ...
          constraints adjCons;
          _confinder.add_adjacent_constraint(_keyframeBlocks, adjCons,
                                             _keyframeBlocks.size());
  ...
  _curSubMapBlock->_lastFrameId =
      _frameBlocks.size() - 1;  //_curFrameBlock->_uniqueId;
  _nAccuFrames += 1;
  if (_lastFrameBlock) {  // update accumulated information
    //_accuTran += calTranslationFromTranmat(relPose);
    _accuTran = (_curFrameBlock->_poseLo.inverse() * _curSubMapBlock->_poseLo)
                    .block<3, 1>(0, 3)
                    .norm();
    Eigen::Matrix4d relPose =
        _lastFrameBlock->_poseLo.inverse() * _curFrameBlock->_poseLo;
    _accuRotDeg += calRotationDegFromTranmat(relPose);
  }

  // printf("Update local map time: %f ms\n", time.toc());
  if (_blockMapManager.judgeNewSubmap(
          _accuTran, _accuRotDeg, _nAccuFrames, _config._maxSubMapAccuTran,
          _config._maxSubMapAccuRot, _config._maxSubMapAccuFrames)) {
    _isNewSubMap = true;
    // time.tic();
    {  // down sample submap
      ...
      _voxelFilter.filter(
          *_curSubMapBlock
               ->_pcDown);  // used for registration while finding loop

      if (config()._isFramewisePGO)  // no need to store _pvList for
                                     // reconstructing map after submap PGO
    ...
    std::cout << "Create new submap [" << _subMapBlocks.size() << "]"
              << std::endl;
    CloudBlockPtr subMapBlock =
        boost::make_shared<CloudBlock>(*_curSubMapBlock);
    ...
    if (_config._isNeedGravAligned) {
      ...
      gravAlignMat.block<3, 3>(0, 0) =
          Eigen::Quaterniond::FromTwoVectors(subMapBlock->_gravity,
      ...
      pcl::transformPointCloud(*subMapBlock->_pcDown,
                               *subMapBlock->_pcDownAligned, gravCorrPose);
      // pcl::transformPointCloud(*subMapBlock->_pcDown,
      // *subMapBlock->_pcDownAligned, subMapBlock->_poseLo);
    }

#if SAVE_MID_FILE
    {  // frame cloud in IMU coordinate
       // PointCloudXYZI::Ptr globalDownCloud(new PointCloudXYZI());
       // pcl::transformPointCloud(*subMapBlock->_pcDown, *globalDownCloud,
       // subMapBlock->_poseLo); _pcdWriter.writeBinary(string(ROOT_DIR) +
       //"PCD/block" + std::to_string(subMapBlock->_idInStrip) + ".pcd",
       //*globalDownCloud); _pcdWriter.writeBinary(string(ROOT_DIR) +
       //"PCD/local_block" + std::to_string(subMapBlock->_idInStrip) + ".pcd",
       //*subMapBlock->_pcDownAligned); _pcdWriter.writeBinary(string(ROOT_DIR)
       //+ "PCD/block" + std::to_string(subMapBlock->_idInStrip) + ".pcd",
       //*subMapBlock->_pcRawWorld);
    }
#endif
    ...
    if (_config._isCorrectRealTime)
      _confinder.cancel_registration_constraint(
          _pgoEdges, _config._minConstraintConfidence,
          _config._maxConstraintSigma);
    // add adjacent edge between current submap and the last submap
    _confinder.add_adjacent_constraint(_subMapBlocks, _pgoEdges,
                                       _subMapBlocks.size());
    // printf("Create new sub map time: %f ms\n", time.toc());

    if (_coolingIndex > 0)  // find registration edges and then do pgo
      std::cout << "No need to detect loop for cooling index: " << _coolingIndex
                << std::endl;
    else {
      ...
      if (_config._isLoopDetectEn) {
        ...
        } else if (_config._detectLoopMethod ==
                   1)  // detect loop closure by contour context method
...
void LoopCloser::run() {
  _bFinished = false;
  _voxelFilter.setLeafSize(_config._voxelDownSize, _config._voxelDownSize,
                           _config._voxelDownSize);
  while (1) {
    _mutexBuf.lock();
    // TicToc time;
    bool hasFrameData = false;
    if (!_frameBuffer.empty() &&
        !_isUpdated)  // Only can _frameBuffer pop out while existed blocks are
                      // contributed into voxelmap in main thread
    {
      ...
      //_cloudUtility.get_cloud_bbx_cpt(_curFrameBlock->_pcDown,
      //_curFrameBlock->_localBound, _curFrameBlock->_localCenter);
      _nAccuOptFrames++;
      hasFrameData = true;
    }
    _mutexBuf.unlock();
    if (hasFrameData) {
      ...
      if (isFindLoop || isGNSSProcessed) {
        if (_config._isCorrectRealTime) {  // correct realtime
          if (config()._isFramewisePGO)
            if (config()._isPgoIncremental)
              frameGraphOptimize();
            else
              correctFrameLoop();
          else if (config()._isPgoIncremental)
            submapGraphOptimize();
          else
            correctSubmapLoop();

          if (isGNSSProcessed) printGNSSError();
        } else if (isGNSSProcessed) {  // not realtime loop mode, while gnss
                                       // input, force to correct realtime
          printGNSSError();
          if (config()._isFramewisePGO)
            if (config()._isPgoIncremental)
              frameGraphOptimize();
            ...
            std::cerr << "Cannot support this global optimization mode"
                      << std::endl;
        ...
        // _coolingIndex = std::max(_frameBuffer.size(),
        // (ulong)_config._coolingSubmapNum); // reset loop test cooling count
        _coolingIndex = _config._coolingSubmapNum;  // wating submap count for
                                                    // detecting next loop
...
bool LoopCloser::detectLoopNN(const CloudBlockPtr block,
                              CloudBlockPtrs& subBlocks) {
  TicToc time;
  ...
  PointCloudXYZI::Ptr globalDownCloud(new PointCloudXYZI());
  pcl::transformPointCloud(*block->_pcDown, *globalDownCloud,
                           block->_poseLo);  // pw=Tw1*p1
  cf.get_cloud_bbx(globalDownCloud, block->_bound);

  constraints currentCandiEdges;
  if (_nAccuOptFrames > _config._numFrameLargeDrift &&
      _config._isOverallSearchingOn)  // expand the loop closure searching area
  {
    isOverallLoopSearchingOn = true;
    regEdgeCount = _confinder.find_overlap_registration_constraint(
        subBlocks, block, currentCandiEdges, 1.5 * _config._neighborSearchDist,
        0.0, _config._minSubmapIdDiff, _config._isSearchNeighbor2d, 3);
  } else  // standard loop closure searching
    regEdgeCount = _confinder.find_overlap_registration_constraint(
        subBlocks, block, currentCandiEdges, _config._neighborSearchDist,
        _config._minIOUThre, _config._minSubmapIdDiff,
        _config._isSearchNeighbor2d, 3);
  // std::cout<<"Overlaped blocks: "<<regEdgeCount<<std::endl;
  if (regEdgeCount < 1) return false;
  ...
  PointCloudXYZI::Ptr globalDownCloud1(new PointCloudXYZI()),
      globalDownCloud2(new PointCloudXYZI()),
      transDownCloud1(new PointCloudXYZI());
  pcl::transformPointCloud(*registrationCon.block1->_pcDown, *globalDownCloud1,
                           registrationCon.block1->_poseLo);
  pcl::transformPointCloud(*registrationCon.block2->_pcDown, *globalDownCloud2,
                           registrationCon.block2->_poseLo);
  // Eigen::Matrix4d Tw2 = registrationCon.block1->_poseLo * initT12;
  // pcl::transformPointCloud(*registrationCon.block2->_pcDown,
  // *transDownCloud1, Tw2);
  //        {
  //            _pcdWriter.writeBinary(string(ROOT_DIR) +
  //            "PCD/condi_block_ori"+to_string(registrationCon.block1->_idInStrip)+".pcd",
  //            *globalDownCloud1); _pcdWriter.writeBinary(string(ROOT_DIR) +
  //            "PCD/trans_block_ori"+to_string(registrationCon.block2->_idInStrip)+".pcd",
  //            *transDownCloud1);
  //        }
  Eigen::Matrix4d gravAlignMat = Eigen::Matrix4d::Identity();
  gravAlignMat.block<3, 3>(0, 0) =
      Eigen::Quaterniond::FromTwoVectors(registrationCon.block1->_gravity,
                                         Eigen::Vector3d{0, 0, -1.0})
          .toRotationMatrix();
  ...
  Eigen::Matrix4d poseAlign3 =
      gravAlignMat *
      getSubMapBlock(registrationCon.block2->_idInStrip - 1)->_poseLo;
  ...
  double dist = (registrationCon.block1->_poseLo.block<3, 1>(0, 3) -
                 registrationCon.block2->_poseLo.block<3, 1>(0, 3))
                    .norm();
  // std::cout<<"Loop candidate dist:"<<dist<<std::endl;

  Eigen::Matrix4d Tw2 = registrationCon.block1->_poseLo * initT12;
  pcl::transformPointCloud(*registrationCon.block2->_pcDown, *transDownCloud1,
                           Tw2);
#if SAVE_MID_FILE
  {
    _pcdWriter.writeBinary(std::string(ROOT_DIR) + "PCD/condi_block" +
                               to_string(registrationCon.block1->_idInStrip) +
                               ".pcd",
                           *globalDownCloud1);
    _pcdWriter.writeBinary(std::string(ROOT_DIR) + "PCD/condi_block" +
                               ...
                               ".pcd",
                           *globalDownCloud2);
    _pcdWriter.writeBinary(
        std::string(ROOT_DIR) + "PCD/trans_block_init" +
            to_string(registrationCon.block2->_idInStrip) + "-" +
            to_string(registrationCon.block1->_idInStrip) + ".pcd",
        *transDownCloud1);
  }
#endif
  ...
  registrationCon.sigma = sqrt(fastVGICPReg.compute_error(
      Eigen::Isometry3d(registrationCon.Trans1_2)));  //马氏距离
  ...
  //        registrationCon.sigma =
  //        sqrt(regVGICP.compute_error(Eigen::Isometry3d(registrationCon.Trans1_2)));//马氏距离
  ...
  std::pair<float, float> distScore =
      _confinder.getFitnessScoreDist(globalDownCloud1, transDownCloud1);
  if (distScore.first < 0.5 || distScore.second > 0.25 ||
      registrationCon.sigma > 100) {
    isConverged = false;
  ...
  //    {
  //        Eigen::Matrix4d
  //        diffTrans=initT12.inverse()*registrationCon.Trans1_2;
  ...
  if (isConverged) {
#if SAVE_MID_FILE
    _pcdWriter.writeBinary(
        std::string(ROOT_DIR) + "PCD/trans_block" +
            to_string(registrationCon.block2->_idInStrip) + "-" +
            to_string(registrationCon.block1->_idInStrip) + ".pcd",
        *transDownCloud1);
#endif
    //            std::cout<<"Trans1_2 after vgicp
    //            registration:\n"<<currentCandiEdges[j].Trans1_2<<std::endl;
    // std::cout<<"VGICP reg done successful!"<<std::endl;
    registrationCon.information_matrix =
        100 * registrationCon.information_matrix;
    ...
    _lcPairs.emplace_back(registrationCon.block1->_idInStrip,
                          registrationCon.block2->_idInStrip);
...
bool LoopCloser::detectLoopContourContext(const CloudBlockPtr blockPtr) {
  ...
  // TicToc time;
  std::shared_ptr<ContourManager> tgtCMPtr =
      _ptrEvaluator->getCurrContourManager(blockPtr, _cmConfig);

#if SAVE_MID_FILE
  for (int i = 0; i < _cmConfig.lv_grads_.size(); i++) {
    std::string f_name = std::string(ROOT_DIR) + "result/layer_img/contour_" +
                         "lv" + std::to_string(i) + "_" + tgtCMPtr->getStrID() +
                         ".png";  // TODO: what should be the str name of scans?
  ...
  _ptrContourDB->queryRangedKNN(tgtCMPtr, _thresLb, _thresUb, ptrCands,
                                candCorr, bevTfs);
  ...
  if (!ptrCands.empty()) {
    if (tgtCMPtr->getIntID() - ptrCands[0]->getIntID() <
        _config._minSubmapIdDiff)
      return false;
#if SAVE_MID_FILE
    // save images of pairs
    std::string f_name = std::string(ROOT_DIR) + "result/match_comp_img/lc_" +
                         tgtCMPtr->getStrID() + "-" + ptrCands[0]->getStrID() +
                         ".png";
    ContourManager::saveMatchedPairImg(f_name, *tgtCMPtr, *ptrCands[0]);
    printf("Image saved: %s-%s\n", tgtCMPtr->getStrID().c_str(),
           ptrCands[0]->getStrID().c_str());
#endif
    //   time.tic();
    Constraint registrationCon;
    registrationCon.block1 =
        _subMapBlocks[ptrCands[0]->getIntID()];  // history map (target)
    registrationCon.block2 = blockPtr;           // current map (source)
    registrationCon.con_type = REGISTRATION;
    registrationCon.Trans1_2 = registrationCon.block1->_poseLo.inverse() *
                               registrationCon.block2->_poseLo;  // T1w*Tw2
    ...
    //        initT12.block<3,1>(0,3)=Eigen::Vector3d
    //        (bestT2d.translation()(0),bestT2d.translation()(1),0);
    ...
    PointCloudXYZI::Ptr globalDownCloud1(new PointCloudXYZI()),
        ...
        transDownCloud1(new PointCloudXYZI());
    Eigen::Matrix4d Tw2 = registrationCon.block1->_poseLo * initT12;
    pcl::transformPointCloud(*registrationCon.block1->_pcDown,
                             *globalDownCloud1,
                             registrationCon.block1->_poseLo);
    pcl::transformPointCloud(*registrationCon.block2->_pcDown,
                             *globalDownCloud2,
    ...
    pcl::transformPointCloud(*registrationCon.block2->_pcDown, *transDownCloud1,
                             Tw2);
#if SAVE_MID_FILE
    _pcdWriter.writeBinary(std::string(ROOT_DIR) + "PCD/trans_block_ori" +
                               to_string(registrationCon.block2->_idInStrip) +
                               ".pcd",
                           *transDownCloud1);
#endif
    //        {
    //            _pcdWriter.writeBinary(std::string(ROOT_DIR) +
    //            "PCD/condi_block_ori"+to_string(registrationCon.block1->_idInStrip)+".pcd",
    //            *globalDownCloud1);
    //            _pcdWriter.writeBinary(std::string(ROOT_DIR) +
    //            "PCD/trans_block_ori"+to_string(registrationCon.block2->_idInStrip)+".pcd",
    ...
    iouShare1 =
        _confinder.calculate_iou(registrationCon.block1->_bound,
                                 registrationCon.block2->_bound, iou1, iou2);
    //        std::cout<<"IOU1/IOU2/iou before
    //        reg:"<<iou1<<"/"<<iou2<<"/"<<iouShare1<<std::endl;
    if (iou1 < 0.5 && iou2 < 0.5) return false;
    if (iouShare1 < 0.4) return false;

    std::pair<float, float> distScore1 =
        _confinder.getFitnessScoreDist(globalDownCloud1, transDownCloud1);
    if (distScore1.first < 0.1) {
      //            std::cerr<<"before vgicp
      //            ratio/dist:"<<distScore1.first<<"/"<<distScore1.second<<std::endl;
      // return false;
    }
    ...
    //        cf.get_cloud_bbx(globalDownCloud1,
    //        registrationCon.block1->_bound); cf.get_cloud_bbx(transDownCloud1,
    //        registrationCon.block2->_bound); double iouShare2 =
    //        _confinder.calculate_iou(registrationCon.block1->_bound,
    //        registrationCon.block2->_bound); std::cout<<"IOU after
    //        reg:"<<iouShare2<<std::endl; if(iouShare2<0.8*iouShare1)
    //        {
    //            isConverged=false;
    //            //nWrongLoop++;
    ...
    std::pair<float, float> distScore2 =
        _confinder.getFitnessScoreDist(globalDownCloud1, transDownCloud1);
    // std::cerr<<"after vgicp
    // ratio/dist:"<<distScore2.first<<"/"<<distScore2.second<<std::endl; if
    // (distScore.first < 0.68 || distScore.second > 0.28)
    if (!isConverged || distScore2.first < 0.45 ||
        distScore2.first < 0.8 * distScore1.first || distScore2.second > 0.25) {
      ...
      if (!isConverged) {
        ...
        distScore2 =
            _confinder.getFitnessScoreDist(globalDownCloud1, transDownCloud1);
        // std::cerr<<"After secondary vgicp
        // ratio/dist:"<<distScore2.first<<"/"<<distScore2.second<<std::endl; if
        // (distScore.first < 0.68 || distScore.second > 0.28)
        if (distScore2.first < 0.45 ||
            distScore2.first < 0.8 * distScore1.first ||
            distScore2.second > 0.25) {
          isConverged = false;
          // std::cout << "Coarse register..."<<std::endl;
        }
      }
    ...
    if (isConverged) {
#if SAVE_MID_FILE
      _pcdWriter.writeBinary(
          string(ROOT_DIR) + "PCD/trans_block" +
              to_string(registrationCon.block2->_idInStrip) + "-" +
              to_string(registrationCon.block1->_idInStrip) + ".pcd",
          *transDownCloud1);
#endif
      ...
      //           std::cout<<"VGICP reg done successful!"<<std::endl;
      // registrationCon.sigma =
      // sqrt(fastVGICPReg.compute_error(Eigen::Isometry3d(registrationCon.Trans1_2)));//马氏距离
...
void LoopCloser::correctSubmapLoop() {
  ...
  if (!strcmp(_config._poseGraphOptimizationMethod.c_str(), "g2o"))
    pgo_successful =
        _pgOptimizer.optimize_pose_graph_g2o(_subMapBlocks, _pgoEdges);
  else if (!strcmp(_config._poseGraphOptimizationMethod.c_str(), "ceres"))
    pgo_successful = _pgOptimizer.optimize_pose_graph_ceres(
        _subMapBlocks, _pgoEdges, _config._tInterSubmapLimit,
        _config._RInterSubmapLimit);
  else if (!strcmp(_config._poseGraphOptimizationMethod.c_str(), "gtsam"))
    pgo_successful = _pgOptimizer.optimize_pose_graph_gtsam(
        _subMapBlocks, _pgoEdges);  // TODO: you'd better use gtsam instead
                                    // (just like lego-loam)
  else                              // default: ceres
    pgo_successful = _pgOptimizer.optimize_pose_graph_ceres(
        _subMapBlocks, _pgoEdges, _config._tInterSubmapLimit,
  ...
  if (pgo_successful) {
    ...
    _loopTrans =
        _subMapBlocks.back()->_poseOptimized *
        _subMapBlocks.back()
            ->_poseLo.inverse();  // Tw'w=Tw'1*T1w   (Tw''w=  Tw''w' * Tw'w)

    _pgOptimizer.update_optimized_nodes(
        _subMapBlocks, true,
        true);  // let _poseLo = = _poseInit = _poseOptimized && update bbx at
                // the same time
    _curSubMapBlock->_poseLo =
        _subMapBlocks.back()->_poseLo;  // update current local map's pose
    ...
    if (_config._isCorrectRealTime) correctFrontendMap(_subMapBlocks);
    //_mutexProcess.unlock();
  } else
...
void LoopCloser::correctFrameLoop() {
  ...
  if (!strcmp(_config._poseGraphOptimizationMethod.c_str(), "g2o"))
    pgo_successful =
        _pgOptimizer.optimize_pose_graph_g2o(_frameBlocks, framewisePgoEdges);
  else if (!strcmp(_config._poseGraphOptimizationMethod.c_str(), "ceres"))
    pgo_successful = _pgOptimizer.optimize_pose_graph_ceres(
        _frameBlocks, framewisePgoEdges, _config._tInterSubmapLimit,
        _config._RInterSubmapLimit);
  else if (!strcmp(_config._poseGraphOptimizationMethod.c_str(), "gtsam"))
    pgo_successful = _pgOptimizer.optimize_pose_graph_gtsam(
        _frameBlocks, framewisePgoEdges);  // TODO: you'd better use gtsam
                                           // instead (just like lego-loam)
  else                                     // default: ceres
    pgo_successful = _pgOptimizer.optimize_pose_graph_ceres(
        _frameBlocks, framewisePgoEdges, _config._tInterSubmapLimit,
        _config._RInterSubmapLimit);
  if (pgo_successful) {
    ...
    _loopTrans =
        _frameBlocks.back()->_poseOptimized *
        _frameBlocks.back()
            ->_poseLo.inverse();  // Tw'w=Tw'1*T1w   (Tw''w=  Tw''w' * Tw'w)
    // std::cout<<"Loop Trans:\n"<<_loopTrans<<std::endl;
    _pgOptimizer.update_optimized_nodes(
        _frameBlocks, false,
        false);  // let _poseLo = _poseInit = _poseOptimized && update bbx at
                 // the same time
    ...
    //_curSubMapBlock->_poseLo = _subMapBlocks.back()->_poseLo; //update current
    // local map's pose
    ...
    if (_config._isCorrectRealTime) correctFrontendMap(_frameBlocks);
    // _mutexProcess.unlock();
  } else
...
bool LoopCloser::constructFrameEdges(constraints& framewisePgoEdges) {
  if (_frameBlocks.empty()) return false;
  ...
  for (int i = 0; i < _frameBlocks.size(); i++) {
    ...
    if (i < _frameBlocks.size() - 1)
      _confinder.add_adjacent_constraint(_frameBlocks, framewisePgoEdges,
                                         i + 2);
  }
...
bool LoopCloser::submapGraphOptimize() {
  ...
  if (pgo_successful) {
    ...
    _loopTrans =
        _keyframeBlocks.back()->_poseOptimized *
        _keyframeBlocks.back()
            ->_poseLo.inverse();  // Tw'w=Tw'1*T1w   (Tw''w=  Tw''w' * Tw'w)
    // std::cout<<"Loop Trans:\n"<<_loopTrans<<std::endl;
    _pgOptimizer.update_optimized_nodes(
        _keyframeBlocks, true,
        false);  // let _poseLo = = _poseInit = _poseOptimized && update bbx at
                 // the same time
    ...
    for (int i = 0; i < frameSize; i++) {
      if (_frameBlocks[i]->_uniqueId == (*keyframeItr)->_uniqueId) {
        ...
        if (keyframeItr < _keyframeBlocks.end() - 1) keyframeItr++;
      } else
        _frameBlocks[i]->_poseLo =
            _frameBlocks[i - 1]->_poseLo * _relPoses[i - 1];
    }
    if (_config._isOutputPCD) {
      updatePgoBlockClouds(_frameBlocks);
      savePgoCloud("Frame-After");
...
void LoopCloser::savePgoCloud(const std::string& suffix) {
  _pcdWriter.writeBinary(std::string(ROOT_DIR) + "PCD/pgo" + suffix + ".pcd",
                         *_pgoCloud);
  _pcdWriter.writeBinary(
      std::string(ROOT_DIR) + "PCD/pgoTraj" + suffix + ".pcd", *_pgoTraj);
}
...
//    for(auto& submap:_subMapBlocks)
//    {
//        std::cout<<"Accumulate submap id:" <<submap->_uniqueId<<", point
//        size:"<<totalSurfPvList.size()<<std::endl; for(auto&
//        pv:submap->_pvList)
//        {
//            pv.pw= submap->_poseOptimized.block<3,3>(0, 0) * pv.pi +
//            submap->_poseOptimized.block<3,1>(0, 3);
//            totalSurfPvList.emplace_back(pv);
//        }
//        PointCloudXYZI::Ptr globalPcDown(new PointCloudXYZI());
//        pcl::transformPointCloud(*submap->_pcDown, *globalPcDown,
//        submap->_poseOptimized);//pw=Tw1*p1
//        *_globalCloudDown+=*globalPcDown;
//    }
//    printf("Rebuilding voxel map...\n");
//    buildVoxelMap(totalSurfPvList, -1, _config._voxelLength,
//    _config._maxLayers, _config._layerPointSizeList,
//                  _config._maxPointsSize, _config._maxCovPointsSize,
//                  _config._minEigenValue, _voxelSurfMap);
...
//        std::cout<<"Loop thread corrected Last frame id:
//        "<<_lastCorrectBlockID<<std::endl; return;
//    }
//
//    _mutexUpdate.lock();
//    //Update voxel map by new blocks in buffer
//    _mutexBuf.lock();//Avoid main thread pushing data in
//	std::cout << "Loop thread correct buffer block size:" <<
//_frameBuffer.size() << std::endl;
//    time.tic() ;
//    totalSurfPvList.clear();
//    for(auto& block: _frameBuffer)
//    {
//        std::cout<<"Fronten suspend: Correct buffer block
//        id:"<<block->_uniqueId<<std::endl;
...
//        {
//            pv.pw= block->_poseLo.block<3,3>(0, 0) * pv.pi +
//            block->_poseLo.block<3,1>(0, 3); totalSurfPvList.emplace_back(pv);
//        }
//        PointCloudXYZI::Ptr globalPcDown(new PointCloudXYZI());
//        pcl::transformPointCloud(*block->_pcDown, *globalPcDown,
//        block->_poseLo);
//        *_globalCloudDown+=*globalPcDown;
//        PointType trajPt;
//        trajPt.x = block->_poseLo(0, 3);
...
//    updateVoxelMap(totalSurfPvList, -1, _config._voxelLength,
//    _config._maxLayers, _config._layerPointSizeList,
//                   _config._maxPointsSize, _config._maxCovPointsSize,
//                   _config._minEigenValue, _voxelSurfMap, boxToDel);
//    _lastCorrectBlockID = _frameBuffer.back()->_uniqueId;
//    _isUpdated=true;
//    std::cout<<"Loop thread corrected Last frame id:
//    "<<_lastCorrectBlockID<<std::endl; printf("Loop thread correct buffer
//    block time: %f ms\n", time.toc()); _mutexBuf.unlock();
//    _mutexUpdate.unlock();
//}
...
void LoopCloser::correctFrontendMap(CloudBlockPtrs allBlocks) {
  // Build voxel map by submaps in pgo
  TicToc time;
  if (!_voxelSurfMap.empty()) _voxelSurfMap.clear();  // rebuild voxel map
  ...
  if (!firstBlock->_pvList.empty()) {
    for (auto& pv : firstBlock->_pvList)
      pv.pw = firstBlock->_poseOptimized.block<3, 3>(0, 0) * pv.pi +
              firstBlock->_poseOptimized.block<3, 1>(0, 3);
    buildVoxelMap(firstBlock->_pvList, -1, _config._voxelLength,
                  _config._maxLayers, _config._layerPointSizeList,
                  _config._maxPointsSize, _config._maxCovPointsSize,
                  _config._minEigenValue, _voxelSurfMap);
  ...
  for (int i = firstBlockIdx; i < totalSize; i++) {
    ...
    if (!block->_pvList.empty()) {
      for (auto& pv : block->_pvList)
        pv.pw = block->_poseOptimized.block<3, 3>(0, 0) * pv.pi +
                block->_poseOptimized.block<3, 1>(0, 3);
      std::vector<BoxPointType> boxToDel;
      updateVoxelMap(block->_pvList, -1, _config._voxelLength,
                     _config._maxLayers, _config._layerPointSizeList,
                     _config._maxPointsSize, _config._maxCovPointsSize,
                     _config._minEigenValue, _voxelSurfMap, boxToDel);
  ...
  if (_frameBuffer.empty()) {
    _isUpdated = true;
    _lastCorrectBlockID =
        getFrameBlock(allBlocks.back()->_lastFrameId)->_uniqueId;
    std::cout << "Loop thread corrected Last frame id: " << _lastCorrectBlockID
              << std::endl;
    return;
  }
  ...
  std::unique_lock<std::mutex> lock2(
      _mutexBuf);  // Avoid main thread pushing data in
  std::cout << "Loop thread correct buffer block size:" << _frameBuffer.size()
            << std::endl;
  time.tic();
  for (auto& block : _frameBuffer) {
    std::cout << "Fronten suspend: Correct buffer block id:" << block->_uniqueId
              << std::endl;
    block->_poseLo = _loopTrans * block->_poseLo;  // Tw2'=Tw1'*T1w*Tw2=Tw1'*T12
    block->_poseInit =
        _loopTrans * block->_poseLo;  // Tw2'=Tw1'*T1w*Tw2=Tw1'*T12
    //
    if (!block->_pvList.empty()) {
      for (auto& pv : block->_pvList)
        pv.pw = block->_poseLo.block<3, 3>(0, 0) * pv.pi +
                block->_poseLo.block<3, 1>(0, 3);
      std::vector<BoxPointType> boxToDel;
      updateVoxelMap(block->_pvList, -1, _config._voxelLength,
                     _config._maxLayers, _config._layerPointSizeList,
                     _config._maxPointsSize, _config._maxCovPointsSize,
...
//        std::cout << "Detecting block1-block2:" <<
//        registrationCon.block1->_idInStrip << "-" <<
//        registrationCon.block2->_idInStrip << std::endl;
//        registrationCon.con_type = REGISTRATION;
//        registrationCon.Trans1_2 = registrationCon.block1->_poseLo.inverse() *
//        registrationCon.block2->_poseLo;
//
//        _voxelFilter.setLeafSize(1, 1, 1);
//        PointCloudXYZI::Ptr downCloud1(new PointCloudXYZI()), downCloud2(new
//        PointCloudXYZI()), downCloud2Trans(new PointCloudXYZI());
...
//
//        _pcdWriter.writeBinary(std::string(ROOT_DIR) + "PCD/downCloud1.pcd",
//        *downCloud1); pcl::transformPointCloud(*downCloud2, *downCloud2Trans,
//        regTrans12);//pw=Tw2*p2 _pcdWriter.writeBinary(std::string(ROOT_DIR) +
//        "PCD/downCloud2.pcd", *downCloud2Trans);
//
//        CRegistration<PointType> cReg;
//        time.tic();
//        int regStatus = cReg.coarse_reg_s4pcs(downCloud1, downCloud2,
//        regTrans12,
//                                              1.5 *
//                                              mutableConfig()._cloudPCANeighRadius,
...
//            pcl::transformPointCloud(*downCloud2, *downCloud2Trans,
//            regTrans12);//pw=Tw2*p2
//            _pcdWriter.writeBinary(std::string(ROOT_DIR) +
//            "PCD/downCloud2_s4pcs.pcd", *downCloud2Trans);
//        }
//
//        CFilter<PointType> cf;
//        ConstraintFinder confinder;
//        PointCloudXYZI::Ptr globalDownCloud1(new PointCloudXYZI()),
//        globalDownCloud2(new PointCloudXYZI()), transDownCloud1(new
//        PointCloudXYZI()); Eigen::Matrix4d Tw2 =
//        registrationCon.block1->_poseLo * regTrans12;
//        pcl::transformPointCloud(*registrationCon.block1->_pcDown,
//        *globalDownCloud1, registrationCon.block1->_poseLo);
//        pcl::transformPointCloud(*registrationCon.block2->_pcDown,
//        *transDownCloud1, Tw2); cf.get_cloud_bbx(globalDownCloud1,
//        registrationCon.block1->_bound); cf.get_cloud_bbx(transDownCloud1,
//        registrationCon.block2->_bound); double iou1 =
//        confinder.calculate_iou(registrationCon.block1->_bound,
//        registrationCon.block2->_bound); std::cout << "IOU before VGICP:" <<
//        iou1 << std::endl; if(iou1<0.5)
//        {
...
//        //assert trans diff
//        isCoverged = _confinder.double_check_tran(regTrans12,
...
//        pcl::transformPointCloud(*registrationCon.block2->_pcDown,
//        *transDownCloud1, Tw2); cf.get_cloud_bbx(transDownCloud1,
//        registrationCon.block2->_bound); double iou2 =
//        confinder.calculate_iou(registrationCon.block1->_bound,
//        registrationCon.block2->_bound); std::cout << "IOU after VGICP:" <<
//        iou2 << std::endl;
//
//        if (iou1 > 1.2 * iou2)
//        {
...
//            std::cout << "PGO add edge: " <<
//            registrationCon.block1->_idInStrip << "-" <<
//            registrationCon.block2->_idInStrip << std::endl;
//            addPgoEdges(registrationCon);
//
//            pcl::transformPointCloud(*downCloud2, *downCloud2Trans,
//            registrationCon.Trans1_2);//pw=Tw2*p2
//            _pcdWriter.writeBinary(std::string(ROOT_DIR) +
//            "PCD/downCloud2_vgicp.pcd", *downCloud2Trans);
//        }
...
bool LoopCloser::processGNSS() {
  if (config()._isPgoIncremental) {  // PGO incremental for a existed global
                                     // pose graph
    if (!config()._isFramewisePGO) {
      if (_gnssFrameIndices.find(_curFrameBlock->_uniqueId) !=
          _gnssFrameIndices.end()) {
        if (_keyframeBlocks.back()->_uniqueId != _curFrameBlock->_uniqueId) {
          std::cout << "Added gnss keyframe:" << _curFrameBlock->_uniqueId
                    << std::endl;
          _keyframeBlocks.emplace_back(_curFrameBlock);
          constraints adjCons;
          _confinder.add_adjacent_constraint(_keyframeBlocks, adjCons,
  ...
  if (gnssItr != _gnssMapBuffer.end()) {
    for (auto& idxGNSSPair : _gnssMapBuffer) {
      ...
      if ((idxGNSSPair.second->pos - gnssFrameBlock->_poseLo.block<3, 1>(0, 3))
              .norm() < config()._gnssMaxError) {
        _pgOptimizer.addGPSConstraint(idxGNSSPair.second, idInStrip);
        isGNSSAdded = true;
      }
    }
...
void LoopCloser::printGNSSError() {
  ...
  for (auto idxGNSSPair : _gnssMap) {
    ...
    if (idInStrip >= 0) {
      ...
      double error =
          (gnssData->pos - frameBlock->_poseLo.block<3, 1>(0, 3)).norm();
